...
function includePage($db, $namespace, $title){
    $a = $db->prepare("SELECT d.`content` FROM `document` AS d INNER JOIN `live_document_list` AS l ON d.docid = l.docid WHERE l.namespace=? AND l.title=? LIMIT 1");
...
class NamuMark {
    ...
    private function htmlScan($text) {
        ...
        if(!empty($this->category)) {
            $result .= '<div id="categories" data-pressdo-doc-category>
                            <h2>분류</h2>
                            <ul>';
            foreach($this->category as $category) {
                $result .= '<li data-pressdo-doc-category>'.$this->linkProcessor(':분류:'.$category.'|'.$category, '[[').'</li>';
            }
            $result .= '</ul></div>';
        }
        return $result;
    ...
    private function bqParser($text, &$offset) {
        ...
        $init = true;
        for($i=$pos;$i<$_wlen;$i=self::seekEndOfLine($text, $i)+1){
    ...
    protected function tableParser($text, &$offset) {
        ...
        */
        ...
        if(self::startsWith($text, '|') && !self::startsWith($text, '||') && $tableinit === true) {
            ...
            $cols = explode('||', implode('|', array_slice($caption, 2)));
            ...
            $cols = array_slice($cols, 1, count($cols) - 2);
            $hasCaption = false;
            $tableinit = false;
            $i += 2;
        ...
        $tdCount = count($cols);
        if(!isset($rowIndex)) $rowIndex = 0;
        if(!isset($colIndex)) $colIndex = 0;

        // 칸단위 처리
        foreach ($cols as $col){
            if(!isset($token['rows'][$rowIndex]))
                $token['rows'][$rowIndex] = ['style' => [], 'cols' => []];
            if(!isset($nextrowspan))
                $nextrowspan = 0;
            $rowspan = $nextrowspan;
            $nextrowspan = 0;
            $tdAttr = [];
            ...
            if($col == "\n"){
                ...
                $token['colstyle'][$colIndex] = [];
                $i += strlen($col) + 2;
                $tdInnerStr = htmlspecialchars_decode($col);
                
                // cell style
                if(preg_match('/^<(.+)>/', htmlspecialchars_decode($tdInnerStr), $match)){
                    ...
                    foreach ($attrs as $attr){
                        ...
                        if(preg_match('/^([^=]*)=([^=]*)$/', $attr, $tbattr)){
                            ...
                            if(
                                ...
                                // 표 속성
                                $tdInnerStr = str_replace('<'.$tbattr[0].'>', '', htmlspecialchars_decode($tdInnerStr));
                            ...
                            $tdInnerStr = str_replace('<'.$tbspan[0].'>', '', htmlspecialchars_decode($tdInnerStr));
                            // <^|n>
                            if($tbspan[1] == '-')
                                $rowspan = $tbspan[2];
                            elseif($tbspan[1] == '|')
                            ...
                            $tdInnerStr = str_replace('<'.$tbalign[0].'>', '', htmlspecialchars_decode($tdInnerStr));
                            // <^|n>
                            if($tbalign[1] == '^')
                            ...
                            switch($attr){
                                case ':':
                                    ...
                                    $tdInnerStr = str_replace('<:>', '', htmlspecialchars_decode($tdInnerStr));
                                    break;
                                case '(':
                ...
                $thiscol = ['text' => $this->blockParser($tdInnerStr), 'style' => $tdAttr];

                array_push($token['rows'][$rowIndex]['cols'], $thiscol);
                
                if($rowspan > 0){
                    $thiscol['rowspan'] = $rowspan;
                    while($rowspan > 1){
                        ++$colIndex;
                        $token['rows'][$rowIndex]['cols'][$colIndex] = 'span';
                        --$rowspan;
                    }
                }
                ++$colIndex;
        ...
        if($token['caption'] !== null)
            $tableInnerStr .= '<caption>'.$tk['caption'].'</caption>';
    ...
    private function formatParser($line) {
        ...
        for($j=0;$j<$line_len;self::nextChar($line,$j)) {
            ...
            if(self::startsWith($line, 'attachment', $j) && preg_match('/attachment:([^\/]*\/)?([^ ]+\.(?:jpg|jpeg|png|gif))(?:\?([^ ]+))?/i', $line, $match, 0, $j)) {
                // 파일
                if($this->imageAsLink)
    ...
    protected function renderProcessor($text, $type) {
        if($type == '{{|') {
            ...
            if(self::startsWithi($text, '#!html')) {
                ...
                $wEngine->uriprefix = $this->urlprefix;

                return '<div '.html_entity_decode($match[0]).'>'.$wEngine->toHtml().'</div>';
    ...
    private function closureProcessor($text, $type) {
        return '<div data-pressdo-wiki-closure>'.$this->formatParser($text).'</div>';
    }

    private function linkProcessor($text, $type) {
        $href = explode('|', $text);
        if(!empty($href[1])){
            if(preg_match('/^\[\[파일:(.+)\]\]$/', $href[1])){
        ...
        if(preg_match('/^https?:\/\//', $href[0])) {
            ...
            $class = '_extlink';
            $target = '_blank';
            $datanm = 'data-pressdo-link-external';
...
#                    echo $html;
                ...
                elseif(preg_match('/^\+([1-5]) (.*)$/', $text, $size)) {
                    ...
                    // 문법 이스케이프
                return '<code data-pressdo-nowiki>' . $text . '</code>';
            }
            return $type.$text.$type;
    }
