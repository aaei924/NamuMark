...
class HTMLRenderer {
    ...
    private function appendResult($value) {
        ...
        if(count($this->resultTemp) === 0)
            ...
            if(is_string($value) && is_string($this->resultTemp[count($this->resultTemp)-1])){
    ...
    private function finalLoop() {
        ...
        $finalFragments = array_map(function($item) {
            $mapcb = function($finalFragments) {
                $result = '';
                for($i = 0; $i < count($finalFragments); $i++) {
                    $result .= $finalFragments[$i];
                }
                return ['html' => $result, 'categories' => $this->categories];
            };
            if(is_string($item))
                return $mapcb($item);
            elseif($item['name'] = 'macro') {
                switch($item['macroName']){
                    ...
                    case '목차':
                        ...
                        return $mapcb($macroContent);
                    case 'include':
                        if(!isset($item['options']) || strlen($item['options']) === 0)
                            return $mapcb('<span class="wikitext-syntax-error">오류 : Include 매크로는 최소한 include할 문서명이 필요합니다.</span>');
                        elseif(!is_string($item['options'][0]))
                            return $mapcb('<span class="wikitext-syntax-error">오류 : include할 문서명이 첫번째로 매크로 매개변수로 전달되어야 합니다.</span>');
                        preg_match('/^('.$_ns['wiki'].'|'.$_ns['category'].'|'.$_ns['trash'].'|'.$_ns['filetrash'].'|'.$_ns['wikioperation'].'|'.$_ns['specialfunction'].'|'.$_ns['vote'].'|'.$_ns['user'].'|'.$_ns['discussion'].'|'.$_ns['include'].'|'.$_ns['template'].'|'.$_ns['file'].'):(.*)$/', $item['options'][0], $get_ns);
                        if(!$get_ns){
                            $_NameSpace = $_ns['document'];
                            $_Title = $item['options'][0];
                        } else {
                            $_NameSpace = $get_ns[1];
                            $_Title = $get_ns[2];
                        }
                        $childPage = new NamuMark([$_NameSpace, $_Title]);
